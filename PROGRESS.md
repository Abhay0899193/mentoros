...
- **Next step (exact):** (1) integration polish: citation-pill + Sources-cited clicks currently just `setActive('knowledge')` — wire them to open that specific source's reading view via `kbStore` (check its API; add an `openSource(sourceId)` action if missing). (2) Runtime verify per Part 8 #5 via **verifier** agent (CDP recipe in Decisions log): ingest `~/Documents/abhay/interview-prep/DSA/patterns` via suggestion card + drag a real PDF; doc-only question ("what does my caching note say about write-through?") → grounded [n]-cited answer; repeat Wi-Fi/Ollama-off → FTS-only still answers/searches; re-ingest idempotent (chunk count stable); restart persistence. (3) **design-reviewer**: KB screen + grounded chat, dark+light+states. (4) Screenshots to scratchpad `/p4/`, commit fixes, **hard check-in** (Phase 4 delivered summary; offer Phase 5 = Coding interview first, one type per window).
