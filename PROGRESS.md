...
## Current status
- **Phase:** 5 IN PROGRESS (user approved past Phase-4 gate) — slice 1 = **Coding interview** (one type per check-in per plan.md §Phase 5). Spec = plan.md §4.5 + interview-prep CLAUDE.md protocol (Frame→Interrogate→Scorecard, hint ladder, §5 spaced-rep grades, scorecard→Memory).
- **Contract LANDED in coreClient.ts:** InterviewType/Language/Phase, InterviewProblem(Meta), InterviewSession(Summary), InterviewTurn, EvalResult(+EvalTestResult), InterviewScorecard(+ScorecardDimension); events `interview.token/status/phase/scorecard`; methods listInterviewProblems/listInterviewSessions/startInterview/getInterviewSession/interviewSend/requestHint/runInterviewTests/finishCoding/endInterview/abandonInterview → routes under `/interview/*`.
- **Next step (exact):** (1) **core-engineer**: `src/core/interview/*` — problem bank (~10 originals-worded LC classics across his 3mc patterns, python+javascript starter+tests), SQLite tables (interview_sessions/turns/attempts/scorecards), eval runner (spawn python3/node harness, JSON protocol, 2s/test timeout), phase-scoped interviewer prompts (llama3.1, zero-praise staff-engineer voice per interview-prep CLAUDE.md §0), hint ladder (3 rungs, logged), endInterview → LLM-JSON scorecard → persist + write memories via existing memory service (skill: pattern confidence; mistake entries) → emit `interview.scorecard`; routes; weakness-targeted recommendation from mistake-type memories. (2) **builder**: launcher (4 gradient-glyph cards, only Coding enabled) + session screen (problem md left / Monaco right — `monaco-editor` npm + vite `?worker` bundling, NO CDN; transcript+mini FallbackOrb bottom-left; Run/Hint/Finish toolbar; results drawer; scorecard overlay w/ memoryWrites chips) + interviewStore + Rail/⌘K wiring. (3) Me: integrate/polish, verifier, design-reviewer, fix, commit, CHECK-IN.
