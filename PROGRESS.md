...
## Current status
- **Phase:** 2+3 COMPLETE and USER-APPROVED. **Phase 4 approved and planned — START IT on next resume.**
- **Next step (exact):** Phase 4 per `~/.claude/plans/read-plan-md-and-eager-blossom.md` Part C. Stage 4a: (1) Fable defines KB contract in `renderer/lib/coreClient.ts` (KbSource, POST /kb/sources ingest + `kb.ingest` event, hybridSearch, `chat.sources` event); (2) core-engineer builds ingest (chunk ~800 chars/overlap, pdf-parse + md/txt, nomic embeddings into existing SQLite vector store + NEW FTS5 table, RRF hybrid search), RAG injection into chat.ts (same pattern as memory recall) with [n] citations, auto-offer interview-prep `DSA/patterns/*` + `System Design/concepts/*` as first sources. Stage 4b: builder does KB library screen (§4.7, grid + drag-drop + search + md reading view); Fable does chat citation pills + live "Sources cited" panel (`ContextPanel.tsx` placeholder exists). Verify per Part 8 #5 (offline cited answer), then hard check-in. Phase 5 after = one interview type per window.
