...
import { AlertCircle, Check, ChevronDown, Cloud, HardDrive, KeyRound } from 'lucide-react';
import { useSettings } from '../../../lib/settingsStore';
import { spring, riseIn, staggerChildren, reduced, dur } from '../../../motion/springs';
import { cn } from '../../../lib/cn';
import type { CloudModelInfo, ModelChoice, ModelSurface, ProvidersInfo } from '../../../lib/coreClient';
...
const SURFACES: { id: ModelSurface; label: string; description: string }[] = [
  ...
  { id: 'interviewer', label: 'Interviewer', description: 'Live interview dialogue' },
  { id: 'scorecard', label: 'Scorecard', description: 'Post-interview grading' },
];
...
function choiceLabel(choice: ModelChoice, providers: ProvidersInfo | null): string {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="text-small font-medium text-ink">Anthropic API key</span>
      ...
      {keyState === 'valid' && anthropic?.keyMask ? (
        <div className="flex items-center gap-3">
          <span className="font-mono text-[12px] text-muted">{anthropic.keyMask}</span>
          <button
      ...
      {keyState === 'invalid' && anthropic?.keyError && <p className="text-[12px] text-danger">{anthropic.keyError}</p>}
      {keyState === 'none' && <p className="text-[12px] text-faint">Stored locally — used only to call Anthropic.</p>}
    </div>
  );
}
        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              className="glass overlay-shadow absolute top-full right-0 z-40 mt-2 w-80 rounded-[14px] bg-surface-1/90 p-2"
            >
              <div className="flex flex-col gap-0.5">
                ...
                {!providers.ollama.reachable ? (
                  <p className="px-2 pb-2 text-[12px] text-faint">Ollama not running</p>
                ) : providers.ollama.models.length === 0 ? (
                  <p className="px-2 pb-2 text-[12px] text-faint">No local models installed</p>
                ) : (
                  providers.ollama.models.map((m) => {
                    const selected = choice.provider === 'ollama' && choice.model === m.model;
                    return (
                      <button
                        key={m.model}
                        role="option"
                        aria-selected={selected}
                        onClick={() => choose({ provider: 'ollama', model: m.model })}
                        className={cn(
                          'flex items-center gap-2 rounded-[8px] px-2 py-1.5 text-left text-[12px] hover:bg-surface-2',
                          selected && 'bg-surface-2',
                        )}
                      >
                        <span className={cn('flex size-4 shrink-0 items-center justify-center', selected ? 'text-iris' : 'text-transparent')}>
                          <Check size={12} strokeWidth={2} />
                        </span>
                        <span className="min-w-0 flex-1 truncate text-ink">{m.label}</span>
                        <span className="shrink-0 text-faint">{formatSize(m.sizeBytes)}</span>
                      </button>
                    );
                  })
                )}
                ...
                {!cloudUsable && (
                  <p className="px-2 pb-1.5 text-[12px] text-faint">Enable cloud + add a valid API key</p>
                )}
                <div className={cn('flex flex-col gap-0.5', !cloudUsable && 'pointer-events-none opacity-45')}>
                  {providers.anthropic.catalog.map((m) => {
                    const selected = choice.provider === 'anthropic' && choice.model === m.model;
                    return (
                      <button
                        key={m.model}
                        role="option"
                        aria-selected={selected}
                        onClick={() => choose({ provider: 'anthropic', model: m.model })}
                        className={cn(
                          'flex flex-col gap-0.5 rounded-[8px] px-2 py-1.5 text-left text-[12px] hover:bg-surface-2',
                          selected && 'bg-surface-2',
                        )}
                      >
                        <span className="flex items-center gap-2">
                          ...
                          <span className="shrink-0 text-faint">{formatCost(m)}</span>
                        </span>
                        <span className="pl-6 text-[11px] text-muted">{m.note}</span>
                      </button>
                    );
                  })}
                </div>
  ...
  return (
    <div className="flex items-center justify-between gap-4 rounded-[10px] px-3 py-2.5">
      <div className="min-w-0">
        ...
        {fallingBack && <p className="mt-0.5 text-[11px] text-faint">Falling back to local — enable cloud + a valid key</p>}
      </div>
      <SurfacePicker surface={surface} choice={choice} providers={providers} cloudUsable={cloudUsable} />
    </div>
  );
}
  ...
  return (
    <Panel title="Models">
      {providersLoading && !providers ? (
        ...
        >
          <motion.div variants={reduced(reduce, riseIn)} className="flex items-start justify-between gap-4 px-3 py-1">
            <div>
              <p className="text-small font-medium text-ink">Use cloud models</p>
            ...
            <motion.div variants={reduced(reduce, riseIn)} className="flex flex-col gap-0.5 border-t border-line pt-3">
              {SURFACES.map((s) => (
                <SurfaceRow
                  ...
                  choice={settings?.models[s.id] ?? { provider: 'ollama', model: providers.ollama.defaultModel }}
                />
              ))}
            </motion.div>
          )}
